...
DFA::DFA(const std::vector<char> &alphabet, const std::vector<const State *> &states, const DFATransition &transition)
        : alphabet(alphabet), states(states), transition(transition) {
...
void DFA::dot(const std::string &path) const {
    ...
    [[maybe_unused]] int res = system(("dot -Tpng " + path + " -o " + pngPath + " >/dev/null 2>&1").c_str());
}
...
DFA *DFA::SSC(const NFA *fa) {
    ...
    while (!queue.empty()) {
        for (const char c : fa->alphabet) {
            ...
            bool accepting = std::find_if(begin(nextStates), end(nextStates),
                                          [](const auto &state) { return state->accepting; }) != nextStates.end();
            newStates.push_back(
                    new State{makeName(nextStates), false, accepting, static_cast<uint32_t>(newStates.size())});
